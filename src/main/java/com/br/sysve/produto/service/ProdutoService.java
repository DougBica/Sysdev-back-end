...
public class ProdutoService {
	...
	public Produto getOne(Long id) throws ResourceNotFoundException {
		return produtoRepository.findById(id).orElseThrow(() -> new ResourceNotFoundException("Employee not found for this id :: " + id ));
	}
	
	public Produto criarEditarProduto(ProdutoDto produtoDto) throws ResourceNotFoundException {
		Produto prod = new Produto();
		if (produtoDto.getId() != null) 		
			prod = produtoRepository.findById(produtoDto.getId()).orElseThrow(() -> new ResourceNotFoundException("Employee not found for this id :: " + produtoDto.getId()));
